...
cartRouter.post('/',async (req,res)=>{    
    let cart_created = await carts.createCart();    
...
cartRouter.get('/:id/products',async (req,res)=>{   
    ...
    let {data, message} = await carts.getProducts(cartId_request);//obtengo los productos del carrito          
...
cartRouter.delete('/:id',async (req,res)=>{ 
    ...
    let result = await carts.deleteCartbyId(cartId_request);//envio el id del carrito a borrar    
    res.send(result);    
})
...
cartRouter.delete('/:id/products/:id_prod',async (req,res)=>{      
    ...
    let {data,message} = await carts.getProducts(cartId_request);  //obtengo los productos del carrito antes de ejecutar el borrado         
    if (!message){//si no se envio un mensaje entonces existen datos para poder procesar                
        let product_found = data.find(prod=>prod===prodId_request);//busco el product ID dentro del array de productos        
        if (product_found){//si encuentro un producto, ejecuto la operacion de borrado del mismo
